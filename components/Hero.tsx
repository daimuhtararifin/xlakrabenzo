...
export const Hero: React.FC = () => {
  ...
  const splitText = (text: string) => {
    return text.split('').map((char, index) => (
      <span key={index} className="inline-block char-reveal opacity-0 translate-y-10">
        {char === ' ' ? '\u00A0' : char}
      </span>
    ));
  };
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-2 gap-8 lg:gap-8 items-center">
          ...
          <div className="flex flex-col items-center lg:items-start text-center lg:text-left z-10 pointer-events-none md:pointer-events-auto order-1 lg:order-none">
            ...
            <h1 ref={headlineRef} className="font-clash font-semibold text-4xl sm:text-5xl md:text-6xl lg:text-7xl leading-[1.1] mb-6 tracking-tight relative z-20">
              ...
              <span className="font-clash font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 via-blue-500 to-cyan-400 bg-[length:200%_auto] animate-gradient">
                {splitText("Harga Hemat")}
              </span>
            </h1>
            ...
            <p ref={subheadRef} className="opacity-0 translate-y-4 font-inter text-gray-400 text-base md:text-lg leading-relaxed max-w-lg mb-8 md:mb-10 relative z-20">
              Satu paket untuk semua kebutuhan. Dapatkan akses Paket Akrab XL Premium dengan harga stabil dan transparan. Dikelola profesional untuk kamu yang anti-ribet.
            </p>

            {/* CTA */}
            <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto pointer-events-auto">
              <MagneticButton>
                <button className="group relative px-8 py-4 bg-gradient-to-r from-[#0055B8] to-[#0284c7] rounded-full text-white font-bold font-inter overflow-hidden shadow-[0_0_20px_rgba(0,85,184,0.4)] transition-all hover:shadow-[0_0_30px_rgba(34,211,238,0.5)] interactive w-full sm:w-auto">
